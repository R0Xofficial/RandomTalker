...
# Bot owner ID
BOT_OWNER_ID = 0000000000  # Replace with the actual bot owner's Telegram user ID
...
async def report(update: Update, context: CallbackContext) -> None:
    ...
    reported_id = int(update.message.text.split()[1])
    reason = ' '.join(update.message.text.split()[2:]) if len(update.message.text.split()) > 2 else 'No reason provided'
    ...
    with report_conn:
        cursor = report_conn.execute(
            ...
            (user_id, reported_id, reason, media_id)
        )
...
async def handle_callback(update: Update, context: CallbackContext) -> None:
    ...
    if action == 'accept':
        ...
        await query.edit_message_text(text=f"Report/Appeal {report_id} has been accepted. User {reported_id} is banned.")
        await context.bot.send_message(reporter_id, f'Your report/appeal (ID: {report_id}) has been accepted.')
        ...
        await context.bot.send_message(reporter_id, f'Your report/appeal (ID: {report_id}) has been rejected.')
